  ...
  static async Abrir({ Prestamo }) {
    ...
    const ContenidoHTML = `
      <div style="margin-bottom: 1.25rem;">
        <div style="font-size: 0.9rem; color: var(--color-texto-secundario);">Cliente</div>
        <div style="font-size: 1.15rem; font-weight: 700;">${Prestamo.NombreCliente} (Préstamo #${Prestamo.Id || Prestamo.IdDocumento})</div>
      </div>

      <div class="TablaResponsiva" style="max-height: 380px; overflow-y: auto;">
        <table class="Tabla">
          <thead>
            <tr>
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 1.25rem; padding-top: 1rem; border-top: 1px solid var(--color-borde);">
        ...
        <span id="TextoTotalAbonadoHistorial" style="font-size: 1.2rem; font-weight: 800; color: var(--color-primario);">Bs. 0.00</span>
      </div>
    `;
        ...
        try {
          ...
          if (ListaPagos.length === 0) {
            CuerpoTabla.innerHTML = `
              <tr>
                <td colspan="3" style="text-align: center; padding: 2rem; color: var(--color-texto-atenuado);">
          ...
          ListaPagos.forEach((Pago) => {
            ...
            FilasHTML += `
              <tr>
                <td><strong>#${Pago.Id || Pago.IdDocumento || "-"}</strong></td>
